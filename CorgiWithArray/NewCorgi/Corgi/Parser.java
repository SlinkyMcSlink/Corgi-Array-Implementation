   ...
   private Node parseStatement() {
      ...
      // --------------->>>   <var> = <expr> or funcCall
         ...
         if ( token.matches("single","=") ) {// assignment
            token = lex.getNextToken();
            if (token.isKind("array") ) { // -----------------------------------------------------------NEW
                token = lex.getNextToken();
                errorCheck( token, "single", "(" );
                Node first = parseExpr();
                token = lex.getNextToken();
                errorCheck( token, "single", ")" );
                return new Node( "createArray", varName, first, null, null );
            }    
            else if (token.isKind("var") ) {
                lex.putBackToken( token );
               Node first = parseExpr();
               return new Node( "fromArray", varName, first, null, null );
            }      // End new----------------------------------------------------------------------------NEW
            else {
               Node first = parseExpr();
         ...
         else if ( token.matches("single","[")) {// funcCall  // ------------------------------------NEW
            Node first = parseExpr();
            token = lex.getNextToken();
            errorCheck( token, "single", "]" );
            token = lex.getNextToken();
            ...
            return new Node("toArray", varName, first, second, null);
         }    // End new-----------------------------------------------------------------------------------------NEW
         else {
...
// ----------------------------------------------------------------------------NEW
            ...
            token = lex.getNextToken();
            errorCheck( token, "single", "]" );
            return new Node("array", name, first, null, null );
