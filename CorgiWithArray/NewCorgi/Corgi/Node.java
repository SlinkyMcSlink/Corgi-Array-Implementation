...
public class Node {
  ...
  public static ArrayList<CorgiArray> arrays = new ArrayList<CorgiArray>();  //-------------------------------NEW
...
//      System.out.println("Executing node " + id + " of kind " + kind );
      ...
      else if ( kind.equals("createArray") ) {
         int size = (int) first.evaluate();
         CorgiArray newArray = new CorgiArray(size);
         arrays.add(newArray);
         table.store( info, (double) arrayid );
         arrayid++;
      }

      else if ( kind.equals("toArray") ) {
         int id = (int) table.retrieve( info );
         int index = (int) first.evaluate();
         double val = second.evaluate();
         arrays.get(id).set(index, val);
         
      }

      else if ( kind.equals("fromArray") ) {
         double value = first.evaluate();
         table.store( info, value );
      }
...
//      System.out.println("Evaluating node " + id + " of kind " + kind );
      ...
      //-------------------------------------NEW------------------------------------------------
         ...
         int ind = (int) first.evaluate();
         int array = (int) table.retrieve(info);
         return arrays.get(array).get(ind);
      }
